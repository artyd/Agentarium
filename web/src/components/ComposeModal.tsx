...
export function ComposeModal() {
  ...
  return (
    <Modal onClose={closeCompose} width={620}>
      <div className="compose-compact" style={{ padding: "16px 22px" }}>
        <div style={{ display: "flex", alignItems: "center", marginBottom: 12 }}>
          <h2 className="fk" style={{ fontWeight: 600, fontSize: 20, flex: 1 }}>{L.compose}</h2>
          <button className="btng" style={{ padding: "6px 10px", boxShadow: "none" }} onClick={closeCompose}><Icon name="x" size={13} /></button>
        </div>

        <div className="lbl2" style={{ marginBottom: 6 }}>{L.postType}</div>
        ...
        <div className="field2">
          <label>{L.body}</label>
          <RichEditor key={`b${formKey}`} value={bodyHtml} onChange={setBodyHtml} placeholder={L.body} minHeight={84} />
        </div>

        {(type === "agent" || type === "project" || type === "skill") && (
          <div className="field2"><label>{L.codeOptional}</label><textarea className="finput" rows={3} value={code} onChange={(e) => setCode(e.target.value)} style={{ fontFamily: "ui-monospace, monospace", minHeight: 0 }} /></div>
        )}
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {images.map((u, i) => (
              <div key={i} style={{ position: "relative" }}>
                <img src={u} alt="" style={{ width: 72, height: 72, objectFit: "cover", borderRadius: 10, border: "var(--sw) solid var(--stroke)", display: "block" }} />
            ...
            {images.length < 4 && (
              <button className="btng" style={{ width: 72, height: 72 }} onClick={() => imgInput.current?.click()}><Icon name="image" size={16} /></button>
            )}
          </div>
